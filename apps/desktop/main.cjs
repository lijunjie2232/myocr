...
// Import Database - use absolute path from node_modules
const Database = require('better-sqlite3');

// Import AI services from packages (will be loaded dynamically)
let ocrService = null;
let summaryService = null;

async function loadAIServices() {
  try {
    const aiCore = await import('../../packages/ai-core/src/index.ts');
    ocrService = aiCore.ocrService;
    summaryService = aiCore.summaryService;
    console.log('[Electron Main] AI services loaded successfully');
  } catch (error) {
    console.error('[Electron Main] Failed to load AI services:', error);
  }
}
...
app.whenReady().then(async () => {
  // Load AI services first
  await loadAIServices();
  
  // Then initialize database and create window
  initDatabase();
  setupIpcHandlers();
  createWindow();
});
